...
func TestCNIPluginFailure(t *testing.T) {
	exitErr := &exec.ExitError{Stderr: []byte("stderr details\n")}
	if got, want := cniPluginFailure([]byte("CNI error JSON\n"), exitErr), "CNI error JSON\nstderr: stderr details"; got != want {
		t.Fatalf("cniPluginFailure() = %q, want %q", got, want)
	}
}
